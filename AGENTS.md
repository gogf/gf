...
| Path | Purpose |
| --- | --- |
| `frame/g`, `frame/gins` | Facade (`g.Server()`, `g.DB()`, `g.Cfg()`) and singleton container |
| `net/` | HTTP, TCP/UDP, load balancing, discovery, tracing, OpenAPI |
| `os/` | Config, CLI, cron, log, files, cache, session, and other OS helpers |
| `database/` | `gdb` ORM core and `gredis` client core |
| `container/` | Concurrent arrays, maps, sets, lists, queues, and typed values |
| `encoding/` | JSON, XML, YAML, TOML, INI, and related codecs |
| `text/` | `gstr` and `gregex` |
| `util/` | Conversion, validation, and helpers (`gconv`, `gvalid`) |
| `crypto/` | AES, DES, RSA, MD5, SHA, CRC32 |
| `errors/` | Stack-aware errors and error codes |
| `internal/` | Private helpers; do not import from outside the root module |
| `test/gtest` | Framework test helpers used across the suite |
...
Canonical AI resources live in `.agents/` and `AGENTS.md`. Per-tool paths such as `CLAUDE.md`, `.claude/skills`, and `.cursor/commands` are local symlinks created by:

```bash
make agents                         # TTY picker
make agents agent=claude            # one-shot link
make agents agent=claude action=unlink
gf agents --agent=claude            # same command from the gf CLI
```

These local paths are gitignored. After cloning, run `make agents agent=<name>` or `gf agents --agent=<name>` for the coding agent you use.
...
- **Do not rewrite existing unit tests just to make new or changed code pass**: existing tests are the primary safeguard of current behavior. If a change would require editing an existing test, strictly review whether that edit is necessary and correct. Do not change existing assertions, fixtures, or control flow merely so that newly added or modified functionality can complete the suite. Prefer adding new tests for the new path. Existing tests may be updated only when the original test is itself incorrect, or when the public contract is intentionally changing and the old assertion would lock in the wrong behavior.
