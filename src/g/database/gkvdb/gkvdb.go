...
// 数据库支持的范围： 2195682563(极端) <= n <= 2301240528000000(理想)

// 数据结构要点   ：数据的分配长度cap >= 数据真实长度len，且 cap - len <= bucket，当数据存储内容发生改变时，依靠碎片管理器对碎片进行回收再利用

// 索引文件结构   ：元数据文件偏移量(5，最大1TB) 元数据文件列表真实长度(5,最大1TB)
// 元数据文件结构1：[数据项长度(11bit) 键值真实长度(28bit,最大256MB) 数据项类型(1bit - 0) 键值(变长,最大5)  键名(变长)](变长,链表)
// 元数据文件结构2：[数据项长度(11bit) 键值真实长度(28bit,最大256MB) 数据项类型(1bit - 1) 数据文件偏移量(5) 键名(变长)](变长,链表)
// 数据文件结构  ：键值(变长)
...
const (
    ...
    gMAX_KEY_SIZE            = (0xFFFF >> 7) - 10         // 键名最大长度(501byte)
    gMAX_VALUE_SIZE          = 0xFFFFFFFF >> 2            // 键值最大长度(1GB=1073741823byte)
    gBUCKET_SIZE             = 64                         // 数据分块大小(byte, 值越大，初始化时占用的空间越大)
...
func (db *DB) PrintState() {
    mtpieces := db.mtsp.GetAllBlocksByIndex()
    dbpieces := db.dbsp.GetAllBlocksByIndex()
    fmt.Println("mt pieces:")
    fmt.Println("     size:", len(mtpieces))
    fmt.Println("     list:", mtpieces)
    fmt.Println("db pieces:")
    fmt.Println("     size:", len(dbpieces))
    fmt.Println("     list:", dbpieces)
}
