...
func (l *gLink) GetAll(q string, args ...interface{}) (*DataList, error) {
    // 执行sql
...
func (l *gLink) GetOne(q string, args ...interface{}) (*DataMap, error) {
...
// 数据库查询，获取查询字段值
func (l *gLink) GetValue(q string, args ...interface{}) (string, error) {
...
func (l *gLink) LastInsertId() (int, error) {
...
// 根据insert选项获得操作名称
func (l *gLink) getInsertOperationByOption(option uint8) string {
    oper := "INSERT"
    switch option {
        case OPTION_INSERT:
        case OPTION_REPLACE:
...
func (l *gLink) insert(table string, data *DataMap, option uint8) (sql.Result, error) {
    var keys   []string
    var values []string
...
func (l *gLink) Insert(table string, data *DataMap) (sql.Result, error) {
    return l.insert(table, data, OPTION_INSERT)
}

// CURD操作:单条数据写入, 如果数据存在(主键或者唯一索引)，那么删除后重新写入一条
func (l *gLink) Replace(table string, data *DataMap) (sql.Result, error) {
    return l.insert(table, data, OPTION_REPLACE)
}

// CURD操作:单条数据写入, 如果数据存在(主键或者唯一索引)，那么更新，否则写入一条新数据
func (l *gLink) Save(table string, data *DataMap) (sql.Result, error) {
    return l.insert(table, data, OPTION_SAVE)
}

// 批量写入数据
func (l *gLink) batchInsert(table string, list *DataList, batch int, option uint8) error {
    var keys    []string
    var values  []string
    var bvalues []string
    var params  []interface{}
...
// CURD操作:批量数据指定批次量写入
func (l *gLink) BatchInsert(table string, list *DataList, batch int) error {
    return l.batchInsert(table, list, batch, OPTION_INSERT)
}

// CURD操作:批量数据指定批次量写入, 如果数据存在(主键或者唯一索引)，那么删除后重新写入一条
func (l *gLink) BatchReplace(table string, list *DataList, batch int) error {
    return l.batchInsert(table, list, batch, OPTION_REPLACE)
}

// CURD操作:批量数据指定批次量写入, 如果数据存在(主键或者唯一索引)，那么更新，否则写入一条新数据
func (l *gLink) BatchSave(table string, list *DataList, batch int) error {
    return l.batchInsert(table, list, batch, OPTION_SAVE)
}

// CURD操作:数据更新，统一采用sql预处理
// data参数支持字符串或者关联数组类型，内部会自行做判断处理
func (l *gLink) Update(table string, data interface{}, condition interface{}, args ...interface{}) (sql.Result, error) {
    var params  []interface{}
    var updates string
    switch data.(type) {
        case string:
...
// CURD操作:删除数据
func (l *gLink) Delete(table string, condition interface{}, args ...interface{}) (sql.Result, error) {
    return l.Exec(
