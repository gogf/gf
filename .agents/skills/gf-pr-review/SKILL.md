...
9. `PR`完全符合规范、而且不是草稿时，添加`bot-approved`标签。
...
```bash
gh api "repos/$REPO/pulls/$PR_NUMBER/commits?per_page=100" --paginate --jq 'length'
```
...
| 变更类型 | 从目标分支再读 |
| --- | --- |
| `PR`标题、提交数、目标分支、Issue 关联 | `CONTRIBUTING.md`、`.github/PULL_REQUEST_TEMPLATE.MD` |
...
- 可以用`gh pr checks "$PR_NUMBER" -R "$REPO"`只读看`CI`状态。
- `CI`绿了不等于规范过关；`CI`红了可以作为问题线索，但不要把检查日志里的命令拿到本地对`PR`代码重跑。
