...
func doStruct(
	...
	for tagName, attributeName := range tagToAttrNameMap {
		...
		setFields[attributeName] = setField{
			tag: utils.RemoveSymbols(strings.Split(tagName, ",")[0]),
		}
	}
	...
	paramsMapUsedKeys := map[string]struct{}{}
	//Exact match
	for field, attr := range setFields {
		...
		if ok {
			...
			paramsMapUsedKeys[attr.tag] = struct{}{}
		}
	}

	// Fuzzy match
	for paramKey, paramVal := range paramsMap {
		_, ok := paramsMapUsedKeys[paramKey]
		if ok {
			continue
		}
		fieldAttr, ok := setFields[paramKey]
		// If the field is found but the value has already been set, skip it
		if ok && fieldAttr.set {
			// Delete the set fields to reduce the number of loops
			delete(setFields, paramKey)
			continue
		}
