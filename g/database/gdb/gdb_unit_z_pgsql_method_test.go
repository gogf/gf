...
func Test_DB_GetSql_Pgsql(t *testing.T) {
	if pgdb == nil {
		return
	}
	gtest.Case(t, func() {
		sqls := pgdb.GetQueriedSqls()
		gtest.Assert(len(sqls), 0)
		...
		_, err = pgdb.Query("SELECT 2")
		gtest.Assert(err, nil)

		sqls = pgdb.GetQueriedSqls()
		gtest.Assert(len(sqls), 2)
	})
}

func Test_DB_Exec_Pgsql(t *testing.T) {
	if pgdb == nil {
		return
	}
	...
	if _, err := pgdb.Exec("ERROR"); err == nil {
		gtest.Fatal("FAIL")
	}
}

func Test_DB_Prepare_Pgsql(t *testing.T) {
	if pgdb == nil {
		return
	}

	st, err := pgdb.Prepare("SELECT 100 aa")
	if err != nil {
		gtest.Fatal(err)
	}
	rows, err := st.Query()
