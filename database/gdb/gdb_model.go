...
func (m *Model) initModelInterface() {
	...
	if impl != nil {
		...
		// Prevent nil pointer
		m.ModelInterface = &DefaultModelInterfaceImpl{
			Model: m,
		}
	}
}
...
//
// See Core.Raw.
func (m DefaultModelInterfaceImpl) Raw(rawSql string, args ...interface{}) *Model {
...
func (m DefaultModelInterfaceImpl) Partition(partitions ...string) *Model {
	model := m.getModel()
...
func (m DefaultModelInterfaceImpl) Ctx(ctx context.Context) *Model {
	if ctx == nil {
		return m.Model
	}
	model := m.getModel()
...
func (m DefaultModelInterfaceImpl) GetCtx() context.Context {
...
func (m DefaultModelInterfaceImpl) As(as string) *Model {
	if m.tables != "" {
		model := m.getModel()
		split := " JOIN "
...
func (m DefaultModelInterfaceImpl) DB(db DB) *Model {
	model := m.getModel()
	model.db = db
	return model
}
...
func (m DefaultModelInterfaceImpl) TX(tx TX) *Model {
	model := m.getModel()
	model.db = tx.GetDB()
	model.tx = tx
	return model
...
func (m DefaultModelInterfaceImpl) Schema(schema string) *Model {
	model := m.getModel()
	model.schema = schema
	return model
}
...
func (m DefaultModelInterfaceImpl) Clone() *Model {
	newModel := &Model{}
	//if m.tx != nil {
...
func (m DefaultModelInterfaceImpl) Master() *Model {
	model := m.getModel()
...
func (m DefaultModelInterfaceImpl) Slave() *Model {
	model := m.getModel()
...
func (m DefaultModelInterfaceImpl) Safe(safe ...bool) *Model {
	if len(safe) > 0 {
		m.safe = safe[0]
	} else {
		m.safe = true
...
func (m DefaultModelInterfaceImpl) Args(args ...interface{}) *Model {
	model := m.getModel()
...
// ModelHandler is a function that handles given Model and returns a new Model that is custom modified.
func (m DefaultModelInterfaceImpl) Handler(handlers ...ModelHandler) *Model {
	model := m.getModel()
