...
type TX struct {
	...
	hookAfterCommit   []hookFunc      // hookAfterCommit is callback functions that execute after transaction commited successful.
	hookAfterRollback []hookFunc      // hookAfterRollback is callback functions that execute after transaction rollbacked successful.
}
...
// HookAfterCommit adds callback functions that execute after transaction commited successful.
func (tx *TX) HookAfterCommit(fn ...hookFunc) {
	tx.hookAfterCommit = append(tx.hookAfterCommit, fn...)
}

// HookAfterRollback adds callback functions that execute after transaction rollbacked successful.
func (tx *TX) HookAfterRollback(fn ...hookFunc) {
	tx.hookAfterRollback = append(tx.hookAfterRollback, fn...)
}
...
func (tx *TX) Commit() error {
	...
	if err == nil {
		for _, fn := range tx.hookAfterCommit {
			fn(tx.ctx)
		}
	}
	return err
...
func (tx *TX) Rollback() error {
	...
	if err == nil {
		for _, fn := range tx.hookAfterRollback {
			fn(tx.ctx)
		}
	}
	return err
