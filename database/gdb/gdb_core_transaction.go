...
type TX struct {
	...
	hookHandlers     map[int][]TransactionHookHandler // Hook functions for transaction hook feature.
}
...
func (tx *TX) Commit() error {
	var handlers []TransactionHookFuncCommit
	if v, ok := tx.hookHandlers[tx.transactionCount]; ok {
		for i := range v {
			if v[i].Commit != nil {
				handlers = append(handlers, v[i].Commit)
			}
		}
	}

	in := &TransactionHookCommitInput{
		internalParamTransactionHookCommit: internalParamTransactionHookCommit{
			internalParamTransactionHook: internalParamTransactionHook{
				tx:               tx,
				handlerCalled:    gset.New(),
...
func (tx *TX) Rollback() error {
	var handlers []TransactionHookFuncRollback
	if v, ok := tx.hookHandlers[tx.transactionCount]; ok {
		for i := range v {
			if v[i].Rollback != nil {
				handlers = append(handlers, v[i].Rollback)
			}
		}
	}

	in := &TransactionHookRollbackInput{
		internalParamTransactionHookRollback: internalParamTransactionHookRollback{
			internalParamTransactionHook: internalParamTransactionHook{
				tx:               tx,
				handlerCalled:    gset.New(),
				TransactionCount: tx.transactionCount,
