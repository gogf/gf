...
// Move Move key from the currently selected database (see SELECT) to the specified destination database.
...
// It may ruin performance when it is executed against large databases. This command is intended for debugging and special operations, such as changing your keyspace layout.
...
func (r *RedisGroupDB) FlushDB(ctx context.Context, options string) (string, error) {
	v, err := r.redis.Do(ctx, "FLUSHDB", options)
	return v.String(), err
}
...
func (r *RedisGroupDB) FlushAll(ctx context.Context, options string) (string, error) {
	v, err := r.redis.Do(ctx, "FLUSHALL", options)
	return v.String(), err
}
