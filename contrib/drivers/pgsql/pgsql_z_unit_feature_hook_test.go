	...
	gtest.C(t, func(t *gtest.T) {
		m := db.Model(table).Hook(
			gdb.BeforeDelete(func(ctx context.Context, in *gdb.HookDeleteInput) error {
				// In new hook design, delete operation always executes between before/after.
				// We make delete a no-op, and run the intended update inside before hook.
				origCondition := in.Condition
				origArgs := append([]any(nil), in.Args...)
				in.Condition = "1=?"
				in.Args = []any{0}
				_, err := in.Model.Data(g.Map{
					"nickname": `deleted`,
				}).Where(origCondition, origArgs...).Update()
				return err
			}),
		)
...
func Test_Model_Hook_Chain(t *testing.T) {
	...
	gtest.C(t, func(t *gtest.T) {
		...
		t.Assert(afterCalls, []string{"hook1", "hook2"})
	})
