...
// Note: PgSQL does not support MySQL's `ON DUPLICATE KEY UPDATE ... VALUES(col)` syntax.
// The equivalent is `INSERT ... ON CONFLICT (col) DO UPDATE SET col = EXCLUDED.col`.
// These tests exercise the same semantic behavior (upsert on unique-key collision) using
// the PgSQL-native syntax; the test function names match the MariaDB baseline per
// project policy (管新不管旧: Layer 2 function names stay identical across drivers).
...
func Test_OnDuplicateKeyUpdate_Basic(t *testing.T) {
	...
	gtest.C(t, func(t *gtest.T) {
		...
		_, err := db.Exec(ctx, fmt.Sprintf(
			"INSERT INTO %s (email, username, score) VALUES (?, ?, ?) ON CONFLICT (email) DO UPDATE SET username = EXCLUDED.username, score = EXCLUDED.score",
			table,
...
func Test_OnDuplicateKeyUpdate_Increment(t *testing.T) {
	...
	gtest.C(t, func(t *gtest.T) {
		// PgSQL: ON CONFLICT needs to reference the target table by name for existing column.
		stmt := fmt.Sprintf(
			"INSERT INTO %s (email, username, login_count) VALUES (?, ?, ?) ON CONFLICT (email) DO UPDATE SET login_count = %s.login_count + 1",
			table, table,
		)
...
func Test_OnDuplicateKeyUpdate_MultipleColumns(t *testing.T) {
	...
	gtest.C(t, func(t *gtest.T) {
		stmt := fmt.Sprintf(
			"INSERT INTO %s (email, username, score, login_count) VALUES (?, ?, ?, ?) ON CONFLICT (email) DO UPDATE SET username = EXCLUDED.username, score = EXCLUDED.score, login_count = %s.login_count + 1",
			table, table,
		)
...
func Test_OnDuplicateKeyUpdate_ConditionalUpdate(t *testing.T) {
	...
	gtest.C(t, func(t *gtest.T) {
		// PgSQL: use GREATEST(EXCLUDED.score, table.score) instead of MySQL's
		// IF(VALUES(score) > score, VALUES(score), score).
		stmt := fmt.Sprintf(
			"INSERT INTO %s (email, username, score) VALUES (?, ?, ?) ON CONFLICT (email) DO UPDATE SET score = GREATEST(EXCLUDED.score, %s.score)",
			table, table,
		)
...
func Test_OnDuplicateKeyUpdate_WithTransaction(t *testing.T) {
	...
	gtest.C(t, func(t *gtest.T) {
		// Transaction with ON CONFLICT DO UPDATE
