...
// FormatUpsert returns SQL clause of type upsert for GaussDB.
// Note: GaussDB is based on PostgreSQL 9.2 which doesn't support ON CONFLICT syntax (introduced in PostgreSQL 9.5).
// Therefore, UPSERT operations (Save, Replace, OnConflict, OnDuplicate) are not supported in GaussDB.
func (d *Driver) FormatUpsert(columns []string, list gdb.List, option gdb.DoInsertOption) (string, error) {
	return "", gerror.NewCode(
		gcode.CodeNotSupported,
		`GaussDB does not support ON CONFLICT (upsert) operations. GaussDB is based on PostgreSQL 9.2, while ON CONFLICT was introduced in PostgreSQL 9.5. Please use separate INSERT and UPDATE operations instead`,
	)
