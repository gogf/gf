...
func init() {
	nodeDefault := gdb.ConfigNode{
		...
		Link:        fmt.Sprintf("tidb:root:%s@tcp(127.0.0.1:4000)/?loc=Local&parseTime=true", TestDbPass),
	...
	nodeInvalid := gdb.ConfigNode{
		Link:        fmt.Sprintf("tidb:root:%s@tcp(127.0.0.1:4010)/?loc=Local&parseTime=true", TestDbPass),
		TranTimeout: time.Second * 3,
	}
