...
func Test_Transaction_Isolation(t *testing.T) {
	...
	gtest.C(t, func(t *gtest.T) {
		...
		// TiDB supports only REPEATABLE READ and READ COMMITTED; SERIALIZABLE is rejected
...
func Test_Transaction_Isolation_Serializable_PhantomRead(t *testing.T) {
	gtest.C(t, func(t *gtest.T) {
		...
		// TiDB has no SERIALIZABLE level; its REPEATABLE READ is snapshot isolation and
		// gives the same freedom from phantom reads this test checks for.
		err := db.TransactionWithOptions(ctx, gdb.TxOptions{
			Propagation: gdb.PropagationRequiresNew,
			Isolation:   sql.LevelRepeatableRead,
		}, func(ctx context.Context, tx1 gdb.TX) error {
			...
			// Insert in another transaction; the snapshot of tx1 must not see it.
