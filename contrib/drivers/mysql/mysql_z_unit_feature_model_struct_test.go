...
func Test_Model_Embedded_Insert(t *testing.T) {
	...
	gtest.C(t, func(t *gtest.T) {
		...
		t.Assert(n, 1)
		value, err := db.Model(table).Fields("passport").Where("id=100").Value(ctx)
		t.AssertNil(err)
...
func Test_Model_Embedded_MapToStruct(t *testing.T) {
	...
	gtest.C(t, func(t *gtest.T) {
		...
		one, err := db.Model(table).Where("id=100").One(ctx)
		t.AssertNil(err)
...
func Test_Structs_Pointer_Attribute(t *testing.T) {
	...
	// All
	gtest.C(t, func(t *gtest.T) {
		one, err := db.Model(table).Where("id < 3").All(ctx)
		t.AssertNil(err)
		users := make([]User, 0)
	...
	gtest.C(t, func(t *gtest.T) {
		users := make([]User, 0)
		err := db.Model(table).Where("id < 3").Scan(ctx, &users)
		t.AssertNil(err)
		t.Assert(len(users), 2)
		t.Assert(*users[0].Id, 1)
		t.Assert(*users[0].Passport, "user_1")
...
func Test_Struct_Empty(t *testing.T) {
	...
	gtest.C(t, func(t *gtest.T) {
		user := new(User)
		err := db.Model(table).Where("id=100").Scan(ctx, user)
...
func Test_Structs_Empty(t *testing.T) {
	...
	gtest.C(t, func(t *gtest.T) {
		all, err := db.Model(table).Where("id>100").All(ctx)
		t.AssertNil(err)
		users := make([]User, 0)
...
func Test_Model_Scan_CustomType_Time(t *testing.T) {
	...
	gtest.C(t, func(t *gtest.T) {
		st := new(MyTimeSt)
		err := db.Model(table).Fields("create_time").Scan(ctx, st)
		t.AssertNil(err)
		t.Assert(st.CreateTime.String(), "2018-10-24 10:00:00")
	})
	gtest.C(t, func(t *gtest.T) {
		var stSlice []*MyTimeSt
		err := db.Model(table).Fields("create_time").Scan(ctx, &stSlice)
		t.AssertNil(err)
...
func Test_Model_Scan_CustomType_String(t *testing.T) {
	...
	gtest.C(t, func(t *gtest.T) {
		st := new(MyStringSt)
		err := db.Model(table).Fields("Passport").WherePri(1).Scan(ctx, st)
		t.AssertNil(err)
		t.Assert(st.Passport, "user_1")
	})
	gtest.C(t, func(t *gtest.T) {
		var sts []MyStringSt
		err := db.Model(table).Fields("Passport").Order("id asc").Scan(ctx, &sts)
		t.AssertNil(err)
...
func Test_Scan_AutoFilteringByStructAttributes(t *testing.T) {
	...
	gtest.C(t, func(t *gtest.T) {
		var user *User
		err := db.Model(table).OrderAsc("id").Scan(ctx, &user)
		t.AssertNil(err)
		t.Assert(user.Id, 1)
	})
	gtest.C(t, func(t *gtest.T) {
		var users []User
		err := db.Model(table).OrderAsc("id").Scan(ctx, &users)
		t.AssertNil(err)
		t.Assert(len(users), TableSize)
		t.Assert(users[0].Id, 1)
	})
