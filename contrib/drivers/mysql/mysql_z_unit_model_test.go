...
func Test_Model_Batch(t *testing.T) {
	...
	gtest.C(t, func(t *gtest.T) {
		...
		r, e := db.Model(table).Data(result).Save(ctx)
		t.Assert(e, nil)
		n, e := r.RowsAffected()
		t.Assert(e, nil)
...
func Test_Model_Update(t *testing.T) {
	...
	// UPDATE...LIMIT
	gtest.C(t, func(t *gtest.T) {
		result, err := db.Model(table).Data("nickname", "T100").Where(1).Order("id desc").Limit(2).Update(ctx)
		t.AssertNil(err)
		n, _ := result.RowsAffected()
		t.Assert(n, 2)
...
func Test_Model_Count(t *testing.T) {
	...
	gtest.C(t, func(t *gtest.T) {
		count, err := db.Model(table).Fields("distinct id,nickname").Where("id>8").Count(ctx)
		t.AssertNil(err)
		t.Assert(count, int64(2))
	})
	// COUNT...LIMIT...
...
func Test_Model_StructsWithOrmTag(t *testing.T) {
	...
	gtest.C(t, func(t *gtest.T) {
		...
		dbInvalid.Model(table).Order("id asc").Scan(ctx, &users)
...
func Test_Model_OrderBy(t *testing.T) {
	...
	gtest.C(t, func(t *gtest.T) {
		result, err := db.Model(table).Order(gdb.Raw("field(id, 10,1,2,3,4,5,6,7,8,9)")).All(ctx)
		t.AssertNil(err)
		t.Assert(len(result), TableSize)
...
func Test_Model_Option_List(t *testing.T) {
	gtest.C(t, func(t *gtest.T) {
		...
		n, _ := r.RowsAffected()
		t.Assert(n, 2)
		list, err := db.Model(table).Order("id asc").All(ctx)
		t.AssertNil(err)
		t.Assert(len(list), 2)
...
func Test_Model_FieldsEx_WithReservedWords(t *testing.T) {
	gtest.C(t, func(t *gtest.T) {
		...
		defer dropTable(table)
		_, err := db.Model(table).FieldsEx("content").One(ctx)
		t.AssertNil(err)
	})
}
...
func Test_Model_Prefix(t *testing.T) {
	...
	// Select.
	gtest.C(t, func(t *gtest.T) {
		r, err := db.Model(table).Where("id in (?)", g.Slice{1, 2}).Order("id asc").All(ctx)
		t.AssertNil(err)
		t.Assert(len(r), 2)
		t.Assert(r[0]["id"], "1")
		t.Assert(r[1]["id"], "2")
	...
	// Select with alias.
	gtest.C(t, func(t *gtest.T) {
		r, err := db.Model(table+" as u").Where("u.id in (?)", g.Slice{1, 2}).Order("u.id asc").All(ctx)
		t.AssertNil(err)
		t.Assert(len(r), 2)
		t.Assert(r[0]["id"], "1")
		t.Assert(r[1]["id"], "2")
	...
	gtest.C(t, func(t *gtest.T) {
		...
		var users []User
		err := db.Model(table+" u").Where("u.id in (?)", g.Slice{1, 5}).Order("u.id asc").Scan(ctx, &users)
		t.AssertNil(err)
	...
	gtest.C(t, func(t *gtest.T) {
		r, err := db.Model(table+" as u1").LeftJoin(table+" as u2", "u2.id=u1.id").Where("u1.id in (?)", g.Slice{1, 2}).Order("u1.id asc").All(ctx)
		t.AssertNil(err)
		t.Assert(len(r), 2)
		t.Assert(r[0]["id"], "1")
		t.Assert(r[1]["id"], "2")
...
func Test_Model_Schema1(t *testing.T) {
	...
	gtest.C(t, func(t *gtest.T) {
		db = db.Schema(TestSchema1)
		r, err := db.Model(table).Data(g.Map{"nickname": "name_100"}).Where("id=1").Update(ctx)
		t.AssertNil(err)
		n, _ := r.RowsAffected()
		t.Assert(n, 1)

		v, err := db.Model(table).Fields("nickname").Where("id=1").Value(ctx)
		t.AssertNil(err)
	...
	// Model.
	gtest.C(t, func(t *gtest.T) {
		v, err := db.Model(table).Schema(TestSchema1).Fields("nickname").Where("id=2").Value(ctx)
		t.AssertNil(err)
		t.Assert(v.String(), "name_2")

		r, err := db.Model(table).Schema(TestSchema1).Data(g.Map{"nickname": "name_200"}).Where("id=2").Update(ctx)
		t.AssertNil(err)
		n, _ := r.RowsAffected()
		t.Assert(n, 1)
		...
		v, err = db.Model(table).Schema(TestSchema2).Fields("nickname").Where("id=2").Value(ctx)
		t.AssertNil(err)
		t.Assert(v.String(), "name_2")
	...
	gtest.C(t, func(t *gtest.T) {
		i := 1000
		_, err := db.Model(table).Schema(TestSchema1).Data(g.Map{
			"id":               i,
		...
		v, err := db.Model(table).Schema(TestSchema1).Fields("nickname").Where("id=?", i).Value(ctx)
		t.AssertNil(err)
		t.Assert(v.String(), "name_1000")

		v, err = db.Model(table).Schema(TestSchema2).Fields("nickname").Where("id=?", i).Value(ctx)
		t.AssertNil(err)
		t.Assert(v.String(), "")
	})
}
...
func Test_Model_Schema2(t *testing.T) {
	...
	// Schema.
	gtest.C(t, func(t *gtest.T) {
		v, err := db.Schema(TestSchema1).Model(table).Fields("nickname").Where("id=2").Value(ctx)
		t.AssertNil(err)
		t.Assert(v.String(), "name_2")

		r, err := db.Schema(TestSchema1).Model(table).Data(g.Map{"nickname": "name_200"}).Where("id=2").Update(ctx)
		t.AssertNil(err)
		n, _ := r.RowsAffected()
		t.Assert(n, 1)
		...
		v, err = db.Schema(TestSchema2).Model(table).Fields("nickname").Where("id=2").Value(ctx)
		t.AssertNil(err)
		t.Assert(v.String(), "name_2")
	...
	gtest.C(t, func(t *gtest.T) {
		i := 1000
		_, err := db.Schema(TestSchema1).Model(table).Data(g.Map{
			"id":               i,
			"passport":         fmt.Sprintf(`user_%d`, i),
			"password":         fmt.Sprintf(`pass_%d`, i),
			"nickname":         fmt.Sprintf(`name_%d`, i),
		...
		v, err := db.Schema(TestSchema1).Model(table).Fields("nickname").Where("id=?", i).Value(ctx)
		t.AssertNil(err)
		t.Assert(v.String(), "name_1000")

		v, err = db.Schema(TestSchema2).Model(table).Fields("nickname").Where("id=?", i).Value(ctx)
		t.AssertNil(err)
		t.Assert(v.String(), "")
	})
}
...
func Test_Model_FieldsExStruct(t *testing.T) {
	...
	gtest.C(t, func(t *gtest.T) {
		...
		r, err := db.Model(table).FieldsEx("create_time, password").OmitEmpty().Data(user).Insert(ctx)
		t.AssertNil(err)
...
func Test_Model_OmitEmpty_Time(t *testing.T) {
	...
	gtest.C(t, func(t *gtest.T) {
		...
		r, err := db.Model(table).OmitEmpty().Data(user).WherePri(1).Update(ctx)
		t.AssertNil(err)
		n, err := r.RowsAffected()
		t.AssertNil(err)
		t.Assert(n, 1)
...
func Test_Model_Having(t *testing.T) {
	...
	gtest.C(t, func(t *gtest.T) {
		all, err := db.Model(table).Where("id > 1").Having("id > 8").All(ctx)
		t.AssertNil(err)
		t.Assert(len(all), 2)
	})
	gtest.C(t, func(t *gtest.T) {
...
func Test_Model_Distinct(t *testing.T) {
	...
	gtest.C(t, func(t *gtest.T) {
		all, err := db.Model(table, "t").Fields("distinct t.id").Where("id > 1").Having("id > 8").All(ctx)
		t.AssertNil(err)
		t.Assert(len(all), 2)
	})
	gtest.C(t, func(t *gtest.T) {
...
func Test_Model_FieldsEx_AutoMapping(t *testing.T) {
	...
	gtest.C(t, func(t *gtest.T) {
		value, err := db.Model(table).FieldsEx("create_date, Passport, Password, NickName, CreateTime").Where("id", 2).Value(ctx)
		t.AssertNil(err)
		t.Assert(value.Int(), 2)
	})

	gtest.C(t, func(t *gtest.T) {
		value, err := db.Model(table).FieldsEx("create_date, ID, Passport, Password, CreateTime").Where("id", 2).Value(ctx)
		t.AssertNil(err)
		t.Assert(value.String(), "name_2")
	})
	// Map
...
func Test_TimeZoneInsert(t *testing.T) {
	...
	gtest.C(t, func(t *gtest.T) {
		_, err = db.Model(tableName).Unscoped().Data(u).Insert(ctx)
		t.AssertNil(err)
		userEntity := &User{}
...
func Test_Model_OnDuplicate(t *testing.T) {
	...
	gtest.C(t, func(t *gtest.T) {
		...
		_, err := db.Model(table).OnDuplicate("passport,password").Data(data).Save(ctx)
		t.AssertNil(err)
		one, err := db.Model(table).WherePri(1).One(ctx)
		t.AssertNil(err)
		t.Assert(one["passport"], data["passport"])
	...
	gtest.C(t, func(t *gtest.T) {
		...
		_, err := db.Model(table).OnDuplicate(g.Slice{"passport", "password"}).Data(data).Save(ctx)
		t.AssertNil(err)
		one, err := db.Model(table).WherePri(1).One(ctx)
		t.AssertNil(err)
		t.Assert(one["passport"], data["passport"])
...
func Test_Model_OnDuplicateEx(t *testing.T) {
	...
	gtest.C(t, func(t *gtest.T) {
		...
		_, err := db.Model(table).OnDuplicateEx("nickname,create_time").Data(data).Save(ctx)
		t.AssertNil(err)
		one, err := db.Model(table).WherePri(1).One(ctx)
		t.AssertNil(err)
		t.Assert(one["passport"], data["passport"])
	...
	gtest.C(t, func(t *gtest.T) {
		...
		_, err := db.Model(table).OnDuplicateEx(g.Slice{"nickname", "create_time"}).Data(data).Save(ctx)
		t.AssertNil(err)
		one, err := db.Model(table).WherePri(1).One(ctx)
		t.AssertNil(err)
		t.Assert(one["passport"], data["passport"])
...
func Test_Builder_OmitEmptyWhere(t *testing.T) {
	...
	defer dropTable(table)
	gtest.C(t, func(t *gtest.T) {
		count, err := db.Model(table).Where("id", 1).Count(ctx)
		t.AssertNil(err)
		t.Assert(count, int64(1))
	})
	gtest.C(t, func(t *gtest.T) {
		count, err := db.Model(table).Where("id", 0).OmitEmptyWhere().Count(ctx)
		t.AssertNil(err)
		t.Assert(count, int64(TableSize))
	})
	gtest.C(t, func(t *gtest.T) {
...
func Test_Scan_Nil_Result_Error(t *testing.T) {
	...
	gtest.C(t, func(t *gtest.T) {
		var s *S
		err := db.Model(table).Where("id", 1).Scan(ctx, &s)
		t.AssertNil(err)
		t.Assert(s.Id, 1)
	})
	gtest.C(t, func(t *gtest.T) {
		var s *S
		err := db.Model(table).Where("id", 100).Scan(ctx, &s)
		t.AssertNil(err)
		t.Assert(s, nil)
	})
	gtest.C(t, func(t *gtest.T) {
	...
	gtest.C(t, func(t *gtest.T) {
		var ss []*S
		err := db.Model(table).Scan(ctx, &ss)
		t.AssertNil(err)
	...
	gtest.C(t, func(t *gtest.T) {
		var ss = make([]*S, 10)
		err := db.Model(table).WhereGT("id", 100).Scan(ctx, &ss)
		t.Assert(err, sql.ErrNoRows)
	})
}
...
func Test_Model_FixGdbJoin(t *testing.T) {
	...
	gtest.C(t, func(t *gtest.T) {
		...
		sqlSlice, err := gdb.CatchSQL(ctx, func(ctx context.Context) error {
			...
			all, err := orm.OrderAsc("src_instance_id").All(ctx)
			t.Assert(len(all), 4)
...
func Test_Model_Year_Date_Time_DateTime_Timestamp(t *testing.T) {
	...
	gtest.C(t, func(t *gtest.T) {
		// insert.
		var now = gtime.Now()
		_, err := db.Model("date_time_example").Data(g.Map{
			"year":      now,
			"date":      now,
			"time":      now,
			"datetime":  now,
...
func Test_OrderBy_Statement_Generated(t *testing.T) {
	gtest.C(t, func(t *gtest.T) {
		...
		sqlArray, _ := gdb.CatchSQL(ctx, func(ctx context.Context) error {
			g.DB("default").Model("employee").Order("name asc", "age desc").All(ctx)
			return nil
		})
...
func Test_Fields_Raw(t *testing.T) {
	gtest.C(t, func(t *gtest.T) {
		table := createInitTable()
		defer dropTable(table)
		one, err := db.Model(table).Fields(gdb.Raw("1")).One(ctx)
		t.AssertNil(err)
		t.Assert(one["1"], 1)

		one, err = db.Model(table).Fields(gdb.Raw("2")).One(ctx)
		t.AssertNil(err)
		t.Assert(one["2"], 2)

		one, err = db.Model(table).Fields(gdb.Raw("2")).Where("id", 2).One(ctx)
		t.AssertNil(err)
		t.Assert(one["2"], 2)

		one, err = db.Model(table).Fields(gdb.Raw("2")).Where("id", 10000000000).One(ctx)
		t.AssertNil(err)
		t.Assert(len(one), 0)
	})
}
