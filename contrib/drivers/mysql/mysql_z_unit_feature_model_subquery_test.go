...
// Test_Model_SubQuery_Correlated tests correlated subquery that references outer table
func Test_Model_SubQuery_Correlated(t *testing.T) {
	table := createInitTable()
	defer dropTable(table)

	gtest.C(t, func(t *gtest.T) {
		// Find users whose id is greater than average id
	...
	gtest.C(t, func(t *gtest.T) {
		...
		subQuery := db.Model(table+" AS inner_table").
			Fields("1").
			Where("inner_table.id = outer_table.id AND inner_table.id <= ?", 3)
...
func Test_Model_SubQuery_Select(t *testing.T) {
	...
	gtest.C(t, func(t *gtest.T) {
		...
		subQuery := db.Model(table).Fields("MAX(id)")
		r, err := db.Model(table).
			Fields("id", "nickname").
			FieldsEx("(?) AS max_id", subQuery).
			Where("id", 1).
			One()
		t.AssertNil(err)
	...
	gtest.C(t, func(t *gtest.T) {
		...
		subQueryMax := db.Model(table).Fields("MAX(id)")
		subQueryMin := db.Model(table).Fields("MIN(id)")

		r, err := db.Model(table).
			Fields("id").
			FieldsEx("(?) AS max_id", subQueryMax).
			FieldsEx("(?) AS min_id", subQueryMin).
			Where("id", 5).
			One()
		t.AssertNil(err)
