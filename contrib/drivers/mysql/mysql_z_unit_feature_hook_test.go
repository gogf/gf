...
// Test_Model_Hook_Chain tests hook chain with data transformation
func Test_Model_Hook_Chain(t *testing.T) {
	table := createTable()
	defer dropTable(table)

	gtest.C(t, func(t *gtest.T) {
		// First hook adds prefix
		m := db.Model(table).Hook(gdb.HookHandler{
			Insert: func(ctx context.Context, in *gdb.HookInsertInput) (result sql.Result, err error) {
				for i := range in.Data {
					in.Data[i]["nickname"] = "prefix_" + fmt.Sprint(in.Data[i]["nickname"])
				}
				return in.Next(ctx)
			},
		})
		...
		m = m.Hook(gdb.HookHandler{
			Insert: func(ctx context.Context, in *gdb.HookInsertInput) (result sql.Result, err error) {
				for i := range in.Data {
					in.Data[i]["nickname"] = fmt.Sprint(in.Data[i]["nickname"]) + "_suffix"
				}
				return in.Next(ctx)
			},
		})
		...
		// Verify both hooks applied transformations
		one, err := db.Model(table).Where("passport", "chain_test").One()
		t.AssertNil(err)
		// Should have both prefix and suffix
		t.Assert(one["nickname"].String(), "prefix_name_suffix")
	})
}
