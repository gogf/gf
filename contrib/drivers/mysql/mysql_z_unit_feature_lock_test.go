...
func Test_Model_Lock(t *testing.T) {
	...
	gtest.C(t, func(t *gtest.T) {
		...
		// Test Lock with FOR SHARE (MySQL 8.0+)
		one, err = db.Model(table).Lock("FOR SHARE").Where("id", 2).One()
		t.AssertNil(err)
		t.Assert(one["id"], 2)

		// Test Lock with legacy LOCK IN SHARE MODE
		...
		// Test Lock with NOWAIT (MySQL 8.0+)
		one, err = db.Model(table).Lock("FOR UPDATE NOWAIT").Where("id", 5).One()
		t.AssertNil(err)
		t.Assert(one["id"], 5)

		// Test Lock with SKIP LOCKED (MySQL 8.0+)
		one, err = db.Model(table).Lock("FOR UPDATE SKIP LOCKED").Where("id", 6).One()
		t.AssertNil(err)
		t.Assert(one["id"], 6)
	})
}
