...
func Test_Model_OmitNil_WithPointerStruct(t *testing.T) {
	...
	gtest.C(t, func(t *gtest.T) {
		// Test OmitNilData with pointer struct
		passport := "ptr_user"
		user := &User{
			Passport: &passport,
			Nickname: nil, // nil pointer, should be omitted
			Password: "ptr_pass",
		}
		result, err := db.Model(table).OmitNilData().Data(user).Where("id", 1).Update()
		t.AssertNil(err)
		n, _ := result.RowsAffected()
		t.Assert(n, 1)
		...
		t.AssertNil(err)
		t.Assert(one["nickname"], "name_1")
		t.Assert(one["passport"], "ptr_user")
	})
}
