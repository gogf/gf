...
func Test_TX_Insert(t *testing.T) {
	...
	gtest.C(t, func(t *gtest.T) {
		...
		_, err = tx.Insert(newCtx, table, g.Map{
			"id":          2,
			"passport":    "t1",
			"password":    "25d55ad283aa400af464c76d713c07ad",
			"nickname":    "T1",
		...
		n, err := tx.Model(table).Count(newCtx)
		t.AssertNil(err)
...
func Test_TX_BatchInsert(t *testing.T) {
	...
	gtest.C(t, func(t *gtest.T) {
		...
		n, err := db.Model(table).Count(newCtx)
		t.AssertNil(err)
...
func Test_TX_BatchReplace(t *testing.T) {
	...
	gtest.C(t, func(t *gtest.T) {
		...
		value, err := db.Model(table).Fields("password").Where("id", 2).Value(ctx)
		t.AssertNil(err)
...
func Test_TX_BatchSave(t *testing.T) {
	...
	gtest.C(t, func(t *gtest.T) {
		...
		value, err := db.Model(table).Fields("password").Where("id", 4).Value(ctx)
		t.AssertNil(err)
...
func Test_TX_GetStruct(t *testing.T) {
	...
	gtest.C(t, func(t *gtest.T) {
		...
		user := new(User)
		err = tx.GetScan(newCtx, user, fmt.Sprintf("SELECT * FROM %s WHERE id=?", table), 3)
		t.AssertNil(err)
...
func Test_TX_GetStructs(t *testing.T) {
	...
	gtest.C(t, func(t *gtest.T) {
		...
		var users []User
		err = tx.GetScan(newCtx, &users, fmt.Sprintf("SELECT * FROM %s WHERE id>=?", table), 1)
		t.AssertNil(err)
...
func Test_Transaction_Propagation(t *testing.T) {
	gtest.C(t, func(t *gtest.T) {
		...
		err := db.Transaction(ctx, func(ctx context.Context, tx gdb.TX) error {
			...
			err = tx.TransactionWithOptions(ctx, gdb.TxOptions{
				...
				_, err := tx2.Insert(ctx, table, g.Map{
					"id":       2,
	...
	gtest.C(t, func(t *gtest.T) {
		...
		err := db.Transaction(ctx, func(ctx context.Context, tx gdb.TX) error {
			...
			err = tx.TransactionWithOptions(ctx, gdb.TxOptions{
				...
				_, _ = tx2.Insert(ctx, table, g.Map{
					"id":       4,
					"passport": "inner_new",
				})
		...
		count, err := db.Model(table).Where("passport", "outer").Count(ctx)
		t.AssertNil(err)
	...
	gtest.C(t, func(t *gtest.T) {
		...
		count, err := db.Model(table).Where("passport", "nested_inner").Count(ctx)
		t.AssertNil(err)
		t.Assert(count, int64(0))

		count, err = db.Model(table).Where("passport IN(?,?)",
			"nested_outer", "nested_after").Count(ctx)
		t.AssertNil(err)
		t.Assert(count, int64(2))
	})
	...
	gtest.C(t, func(t *gtest.T) {
		...
		count, err := db.Model(table).Where("passport", "tx_record").Count(ctx)
		t.AssertNil(err)
		t.Assert(count, int64(0))

		count, err = db.Model(table).Where("passport", "non_tx_record").Count(ctx)
		t.AssertNil(err)
		t.Assert(count, int64(1))
	})
...
func Test_Transaction_Propagation_PropagationSupports(t *testing.T) {
	gtest.C(t, func(t *gtest.T) {
		...
		// 查询结果
		result, err := db.Model(table).OrderAsc("id").All(ctx)
		t.AssertNil(err)
		t.Assert(len(result), 1)
...
func Test_Transaction_Propagation_Complex(t *testing.T) {
	gtest.C(t, func(t *gtest.T) {
		...
		err := db.Transaction(ctx, func(ctx context.Context, tx1 gdb.TX) error {
			// Insert in outer transaction
			_, err := tx1.Insert(ctx, table1, g.Map{
				"id":       1,
				"passport": "outer",
			})
			t.AssertNil(err)
			...
			err = tx1.TransactionWithOptions(ctx, gdb.TxOptions{
				...
				_, err = tx2.Insert(ctx, table1, g.Map{
					"id":       2,
					"passport": "nested1",
				})
				t.AssertNil(err)
				...
				err = tx2.TransactionWithOptions(ctx, gdb.TxOptions{
					...
					_, _ = tx3.Insert(ctx, table1, g.Map{
						"id":       3,
						"passport": "new1",
					})
		...
		count, err := db.Model(table1).Where("passport", "outer").Count(ctx)
		t.AssertNil(err)
		t.Assert(count, int64(1))

		// 2. "nested1" should not exist (rolled back due to error)
		count, err = db.Model(table1).Where("passport", "nested1").Count(ctx)
		t.AssertNil(err)
		t.Assert(count, int64(0))

		// 3. "new1" should not exist (rolled back independently)
		count, err = db.Model(table1).Where("passport", "new1").Count(ctx)
		t.AssertNil(err)
		t.Assert(count, int64(0))

		// 4. "nested2" should not exist (rolled back to savepoint)
		count, err = db.Model(table1).Where("passport", "nested2").Count(ctx)
		t.AssertNil(err)
		t.Assert(count, int64(0))

		// 5. "not_supported" should exist (non-transactional)
		count, err = db.Model(table2).Where("passport", "not_supported").Count(ctx)
		t.AssertNil(err)
		t.Assert(count, int64(1))
	})
	...
	gtest.C(t, func(t *gtest.T) {
		...
		err := db.Transaction(ctx, func(ctx context.Context, tx1 gdb.TX) error {
			// Insert in outer transaction
			_, err := tx1.Insert(ctx, table, g.Map{
				"id":          6,
			...
			err = tx1.TransactionWithOptions(ctx, gdb.TxOptions{
				...
				return db.Transaction(ctx, func(ctx context.Context, tx3 gdb.TX) error {
					_, err := tx3.Insert(ctx, table, g.Map{
						"id":          7,
		...
		count, err := db.Model(table).Where("passport IN(?,?)",
			"suspend_outer", "suspend_resume").Count(ctx)
		t.AssertNil(err)
		t.Assert(count, int64(0))

		// 2. "independent" should exist (committed independently)
		count, err = db.Model(table).Where("passport", "independent").Count(ctx)
		t.AssertNil(err)
		t.Assert(count, int64(1))
	})
}
...
func Test_Transaction_ReadOnly(t *testing.T) {
	...
	gtest.C(t, func(t *gtest.T) {
		...
		err := db.TransactionWithOptions(ctx, gdb.TxOptions{
			...
			_, err := tx.Update(ctx, table, g.Map{"passport": "changed"}, "id=1")
			// Should return error
			return err
		})
		t.AssertNE(err, nil)
		...
		v, err := db.Model(table).Where("id=1").Fields("passport").Value(ctx)
		t.AssertNil(err)
...
func Test_Transaction_Isolation(t *testing.T) {
	...
	gtest.C(t, func(t *gtest.T) {
		...
		err := db.TransactionWithOptions(ctx, gdb.TxOptions{
			...
			_, err := tx1.Update(ctx, table, g.Map{"passport": "dirty_read"}, "id=1")
			t.AssertNil(err)
			...
			err = db.TransactionWithOptions(ctx, gdb.TxOptions{
				...
				v, err := tx2.Model(table).Where("id=1").Fields("passport").Value(ctx)
				t.AssertNil(err)
	...
	gtest.C(t, func(t *gtest.T) {
		...
		err := db.TransactionWithOptions(ctx, gdb.TxOptions{
			...
			// First read
			v1, err := tx1.Model(table).Where("id=1").Fields("passport").Value(ctx)
			t.AssertNil(err)
			...
			err = db.TransactionWithOptions(ctx, gdb.TxOptions{
				...
				_, err := tx2.Update(ctx, table, g.Map{
			...
			v, err := db.Model(table).Where("id=1").Fields("passport").Value(context.Background())
			t.AssertNil(err)
			t.Assert(v.String(), "changed_value")

			// Should still see old value in REPEATABLE READ transaction
			v2, err := tx1.Model(table).Where("id=1").Fields("passport").Value(ctx)
			t.AssertNil(err)
			t.Assert(v2.String(), initialValue)

			// Even after multiple reads, should still see the same value
			v3, err := tx1.Model(table).Where("id=1").Fields("passport").Value(ctx)
			t.AssertNil(err)
	...
	gtest.C(t, func(t *gtest.T) {
		...
		err := db.TransactionWithOptions(ctx, gdb.TxOptions{
			...
			err = db.TransactionWithOptions(ctx, gdb.TxOptions{
				...
				_, err := tx2.Update(ctx, table, g.Map{"passport": "committed_value"}, "id=1")
				return err
			})
			t.AssertNil(err)
...
func Test_Transaction_Spread(t *testing.T) {
	...
	gtest.C(t, func(t *gtest.T) {
		...
		all, err := tx.Model(table).All(newCtx)
		t.AssertNil(err)
		...
		all, err = db.Model(table).All(newCtx)
		t.AssertNil(err)
		t.Assert(len(all), 0)
	})
}
