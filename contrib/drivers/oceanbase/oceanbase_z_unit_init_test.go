...
func init() {
	nodeDefault := gdb.ConfigNode{
		...
		Link:        fmt.Sprintf("oceanbase:root@test:%s@tcp(127.0.0.1:2881)/?loc=Local&parseTime=true", TestDbPass),
	...
	nodeInvalid := gdb.ConfigNode{
		Link:        fmt.Sprintf("oceanbase:root@test:%s@tcp(127.0.0.1:2891)/?loc=Local&parseTime=true", TestDbPass),
		TranTimeout: time.Second * 3,
	}
