...
func Test_Issue2231(t *testing.T) {
	var (
		...
		link    = `mariadb:root:12345678@tcp(127.0.0.1:3307)/a正bc式?loc=Local&parseTime=true`
	)
	gtest.C(t, func(t *gtest.T) {
		match, err := gregex.MatchString(pattern, link)
		t.AssertNil(err)
		t.Assert(match[1], "mariadb")
		t.Assert(match[2], "root")
		t.Assert(match[3], "12345678")
		t.Assert(match[4], "tcp")
		t.Assert(match[5], "127.0.0.1:3307")
...
// Known bug: cross-database JOIN with soft-delete generates unqualified
// `deleted_at` column causing ambiguity error in MariaDB driver.
// See https://github.com/gogf/gf/issues/4725
...
func Test_Issue2552_ClearTableFieldsAll(t *testing.T) {
	...
	// MariaDB driver queries table fields via information_schema.COLUMNS
	// instead of MySQL's "SHOW FULL COLUMNS FROM".
	showTableKey := `information_schema.COLUMNS`
	gtest.C(t, func(t *gtest.T) {
