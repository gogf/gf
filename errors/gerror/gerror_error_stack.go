...
type stackInfo struct {
	// bufs [][]byte
	// bufptrs[0] = unsafe.Pointer(&bufs[0])
	// bufptrs[1] = unsafe.Pointer(&bufs[1])
	// ...
...
func getErrorStackInfo(err *Error, id int, isStackModeBrief bool) (errStackInfo stackInfo) {
	...
	for i, pc := range pcs {
		...
		bufptr, flag := cacheStackTrace.getBytesPtr(pc)
		if bufptr == nil {
			bufptr, flag = addStackInfoToCache(pc)
		}
		count += len(*bufptr)
		...
		if isStackModeBrief {
			if flag == stackObjectFlagStackFilterKeyForGoFrame {
				continue
			}
			errStackInfo.bufptrs = append(errStackInfo.bufptrs, unsafe.Pointer(bufptr))
		} else if flag == stackObjectFlagStackFilterKeyLocal {
			// continue
		} else {
			errStackInfo.bufptrs = append(errStackInfo.bufptrs, unsafe.Pointer(bufptr))
		}
...
func addStackInfoToCache(pc uintptr) (*[]byte, stackObjectFlag) {
	pcs := [1]uintptr{pc}
	...
	bufptr := cacheStackTrace.addAndGetBytesPtr(pc, buf, flag)
	return bufptr, flag
}
