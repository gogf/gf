...
3. Database support
   ...
   | `pgsql` | yes | - |
   | `sqlite` | yes | - |
   | `sqlitecgo` | no | 32-bit `sqlite`: import in `cmd_gen_dao.go` and rebuild. |
   | `clickhouse` | yes | - |
   | `dm` | no | Import in `cmd_gen_dao.go` and rebuild. |
...
COMMAND
    ...
    agents     link local AI coding-agent paths to .agents and AGENTS.md
